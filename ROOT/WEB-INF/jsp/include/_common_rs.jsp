<link rel="stylesheet" type="text/css" href="${rs}/css/ztask.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/jquery.errbox.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/jquery.masker.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/jquery.droplist.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/jquery.pop.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/jquery.calendar.css"/>

<link rel="stylesheet" type="text/css" href="${rs}/css/ztask_task.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/ztask_detail.css"/>
<link rel="stylesheet" type="text/css" href="${rs}/css/ztask_hierachy.css"/>

<script language="JavaScript" src="${rs}/js/jquery.js"></script>
<script language="JavaScript" src="${rs}/js/jquery-ui.js"></script>
<script language="JavaScript" src="${rs}/js/jquery.masker.js"></script>
<script language="JavaScript" src="${rs}/js/jquery.droplist.js"></script>
<script language="JavaScript" src="${rs}/js/jquery.pop.js"></script>
<script language="JavaScript" src="${rs}/js/jquery.calendar.js"></script>
<script language="JavaScript" src="${rs}/js/jquery.toggleInput.js"></script>
<script language="JavaScript" src="${rs}/js/jquery.errbox.js"></script>
<script language="JavaScript" src="${rs}/js/z.js"></script>
<script language="JavaScript" src="${rs}/js/ajax.js"></script>

<script language="JavaScript" src="${rs}/js/ztask_hierachy.js"></script>
<script language="JavaScript" src="${rs}/js/ztask_utils.js"></script>
<script language="JavaScript" src="${rs}/js/ztask_task_detail.js"></script>