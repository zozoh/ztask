...
window.z = {
    ...
    //---------------------------------------------------------------------------------------
    removeIt: function(jq, to) {
        jq = $(jq);
        // 移动 me 到相应的队列
        var w = jq.width();
        // 增加占位对象，以及移动 me
        var holder = $('<div style="display:inline-block;">&nbsp;</div>').width(jq.width()).insertAfter(jq);
        if(to)
            jq.appendTo(to);
        else
            jq.remove();
    ...
    blinkIt: function(jq) {
        jq = $(jq);
        var lg = $('<div class="bxitem">&nbsp;</div>');
        lg.width(jq.width() + 8);
        lg.height(jq.height() + 8);
        lg.insertBefore(jq.children().first())
        lg.css({
        'border-color': "#FF0",
        background: "#FFA",
        opacity: 0.8,
    ...
    //---------------------------------------------------------------------------------------
    // this 为任何可以有子元素的 DOM 或者 jq，本函数在该元素的位置绘制一个 input 框，让用户输入新值
    // callback(newval, oldval) 为回调函数，如果有，用这个函数来格式化输入的值，否则将接受任何输入的值
    //   - callback 的 this 为当前元素，它被调用的时候，输入框还没有消失
    //   - 如果调用者自己实现了 callback，则返回 false 表示新的值不对，所以会继续采用旧的值
    //          否则如果返回值不是 "undefined"，会用返回值自动更新宿主对象的 text
    editIt: function(callback, w, h) {
        // 定义处理函数
        var func = function() {
            var me = $(this);
            var ta = me.parent();
            ...
            if( typeof callback == "function") {
                ...
                var re = callback.apply(ta, [newval, oldval]);
                if(false != re) {
                    var retype = typeof re;
                    txt = retype == "undefined" || retype == "boolean" ? newval : re;
                    ta.html(txt);
                }
            }
            me.unbind().remove();
        ...
        if(!w || w <= 0)
            w = me.outerWidth();
        // 显示输入框
        $('<input class="edit-it" value="' + val + '" oldval="' + val + '">').prependTo(me).css({
        "width": w,
        "height": h,
        "line-height": h + "px",
        "position": "absolute"
        }).data("callback", callback).one("blur", func).one("change", func).select();
    },
    //---------------------------------------------------------------------------------------
    // 将任意 JS 对象转化成 JSON 串
