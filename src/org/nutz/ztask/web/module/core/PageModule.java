...
public class PageModule {
	...
	public void showReportPage(@Param("yy") int year, ServletRequest req) {
		Calendar now = ZTasks.C(ZTasks.now());
		now.set(Calendar.DAY_OF_WEEK, this.reportDay);
		String thisWeek = ZTasks.SD(now.getTime());
		...
		for (int i = 1; i < 13; i++) {
			...
			for (int w = 0; w < weekNumber; w++) {
				...
				boolean isCurrentWeek = thisWeek.equals(ZTasks.SD(c.getTime()));
				...
				map.putAll(Lang.mapf(	"{w:%d, wyy:%d, d:'%s', report:%s, current:%s}",
										w,
										weekOfYear,
										ZTasks.SD(c.getTime()).substring(5),
										hasReport,
										isCurrentWeek));
				cells[i][w] = map;
			}
